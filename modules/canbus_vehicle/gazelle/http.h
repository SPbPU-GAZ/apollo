...
// HTTP_SOCKET http_internal_connect( char const* address, char const* port, uint64_t timeout_ms)
// {   
//     // set up hints for getaddrinfo
//     struct addrinfo hints;
//     memset( &hints, 0, sizeof( hints ) );
...
//     // connect to server
//     res = connect(sock, addri->ai_addr, (int)addri->ai_addrlen);
//     if(res == -1)
//     {
//         bool failed = true;
...
//         {
//             // Set select events
//             fd_set wr_events; FD_ZERO(&wr_events);
//             fd_set ex_events; FD_ZERO(&ex_events);

//             FD_SET(sock, &wr_events);
//             FD_SET(sock, &ex_events);

//             // Set select timeout
//             timeval timeout;
//             const auto seconds = timeout_ms / 1000;
//             timeout.tv_sec = seconds;
//             timeout.tv_usec = (timeout_ms - seconds * 1000) * 1000;

//             res = select(sock + 1, 0, &wr_events, &ex_events, (timeout_ms == 0) ? NULL : &timeout);
//             if (res == -1)
//             {
//                 // LOG_ERROR(LOG_TAG "[{0}] Failed to select for {1}:{2}", m_name, host, port);
//             }
//             if (res == 0)
//             {
//                 //LOG_ERROR(LOG_TAG "[{0}] Connect timed out for {1}:{2}", m_name, host, port);
//             }
//             else
//             {
// #ifdef _WIN32
...
//                 errno = 0;
// #endif
//                 if (FD_ISSET(sock, &wr_events))
//                 {
//                     failed = false;
//                 }

//                 if (FD_ISSET(sock, &ex_events))
//                 {
//                     failed = true;
//                     // LOG_ERROR(LOG_TAG "[{0}] Non-blocking connection attempt failed. Server {1}:{2}", m_name, host, port);
//                 }
//             }
//         }
...
// #ifndef _WIN32
//             int conn_err = 0;
//             socklen_t len = sizeof(conn_err);
//             int retval = getsockopt(sock, SOL_SOCKET, SO_ERROR, &conn_err, &len);

//             if (retval != 0)
//             {
//                 // LOG_ERROR(LOG_TAG "[{0}] Failed to get socket error code: {1}", m_name, strerror(retval));
//                 freeaddrinfo( addri );
//                 close(sock);
//                 return HTTP_INVALID_SOCKET;
//             }
...
//             if (conn_err != 0)
//             {
//                 // LOG_ERROR(LOG_TAG "[{0}] Socket has a non zero error status: {1}", m_name, strerror(conn_err));
//                 freeaddrinfo( addri );
//                 close(sock);
//                 return HTTP_INVALID_SOCKET;
//             }
...
//     assert(sock != HTTP_INVALID_SOCKET);
//     freeaddrinfo( addri );
//     return sock;
// }
...
http_status_t http_process( http_t* http )
    ...
    //         }
        ...
        int res = select( (int)( internal->socket + 1 ), NULL, &sockets_to_check, NULL, &timeout );
        if(res == -1)
        {
        ...
        if( res == 1 )
        {
            int opt = -1;
            socklen_t len = sizeof( opt );
            int retval = getsockopt( internal->socket, SOL_SOCKET, SO_ERROR, (char*)( &opt ), &len);
